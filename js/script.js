...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function updateRadialSpokes() {
    ...
    var GAP = 20; // 18-24px clean gap before track box
    ...
    for (var i = 1; i <= 8; i++) {
      ...
      // Find intersection with expanded track bounding box with GAP
      var xLeft = bxMin - GAP;
      var xRight = bxMax + GAP;
      var yTop = byMin - GAP;
      var yBottom = byMax + GAP;

      var validT = [];

      // Test vertical left edge
      if (cos > 0.0001) {
        var t1 = (xLeft - cx) / cos;
        var yHit1 = cy + t1 * sin;
        if (t1 > hubRadius && yHit1 >= yTop - 1 && yHit1 <= yBottom + 1) {
          validT.push(t1);
        }
      }
      // Test vertical right edge
      if (cos < -0.0001) {
        var t2 = (xRight - cx) / cos;
        var yHit2 = cy + t2 * sin;
        if (t2 > hubRadius && yHit2 >= yTop - 1 && yHit2 <= yBottom + 1) {
          validT.push(t2);
        }
      }
      // Test horizontal top edge
      if (sin > 0.0001) {
        var t3 = (yTop - cy) / sin;
        var xHit3 = cx + t3 * cos;
        if (t3 > hubRadius && xHit3 >= xLeft - 1 && xHit3 <= xRight + 1) {
          validT.push(t3);
        }
      }
      // Test horizontal bottom edge
      if (sin < -0.0001) {
        var t4 = (yBottom - cy) / sin;
        var xHit4 = cx + t4 * cos;
        if (t4 > hubRadius && xHit4 >= xLeft - 1 && xHit4 <= xRight + 1) {
          validT.push(t4);
        }
      }

      var tMin = validT.length > 0 ? Math.min.apply(null, validT) : null;

      if (tMin === null || tMin <= hubRadius) {
        tMin = Math.max(hubRadius + 10, Math.sqrt((bx - cx) * (bx - cx) + (by - cy) * (by - cy)) - 55);
      }
